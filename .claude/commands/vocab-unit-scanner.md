...
- **If the word is a verb (`partOfSpeech` resolves to `"verb"` via `normalizePos()`, including compounds like `"noun/verb"` or `"verb phrase"`), the 3 sentences must be Infinitive / Past / Past participle, not just 3 sentences in the same tense.** `example` = base form, `additionalExamples[0]` = simple past, `additionalExamples[1]` = past participle (perfect or passive). Use `getVerbForms()` (`src/data/verbForms.ts`) for the correct past/past-participle spelling — it handles irregulars and multi-word phrasal verbs. Irregular verbs' past tense (e.g. "fell" for "fall") don't literally contain the base word, but that's fine now: `WordCard.tsx` and `validate-content.mjs` both accept a verb's computed past/past-participle forms as valid highlight/validation targets, so write the grammatically correct sentence rather than avoiding irregulars. This was a gap across 33 verb words in Units 6-9 (all had 3 same-tense sentences, not the 3 forms) fixed after the user pointed at the Forms chips and asked for this explicitly — build it this way from the start.
...
- [ ] Every verb-type word's 3 sentences are Infinitive / Past / Past participle, not 3 sentences in the same tense
