...
Unit 9 ("The Science of Fun") is in progress — opener, vocab-1, song, grammar-1, vocab-2, grammar-2, reading, writing, mission, and project have teacher + learner pairs; book-reading is a teacher deck only so far (no Leo app yet). The Units 7-9 checkpoint band also has its two teacher decks: Review 7-9 and Extra Reading 7-9 (Leonardo da Vinci). Remaining for this unit: the Leo apps for book-reading, review, and extra-reading, per `docs/build-order.md`.

The Mission and Project pages introduce words that are not in `vocabulary.json` yet — `share`, `evaluate`, `unexpected`, `curious`, `thermal image`, `think creatively`, `think critically`, `physicist` (Mission, LP p.159) and `research`, `upside down`, `attach`, `combine`, `thaumatrope`, `optical illusion`, `predict` (Project, LP p.160). Their teacher decks and Leo apps carry the wording and Japanese; adding them as reference cards (light schema for the content/related words, the full rich schema for the three academic ones) with index wiring is still to do.
