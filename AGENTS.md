...
11a. **A schema change is not done until it is applied.** Adding a table or column to `supabase/schema.sql` changes a file, nothing else — the live project is untouched until someone runs it. Every cloud write falls back to localStorage on failure, so a missing table looks exactly like everything working. Apply and verify in the same change; see `docs/supabase.md`. **A new table also needs its `grant … to anon` line** — `test_attempts` shipped live with RLS and both policies and was still unusable from the browser without it. And verify as `anon`, not as the privileged role: a plain `execute_sql` check bypasses grants and RLS, so it proves the table exists and nothing about the path the app takes.
...
Our World also has checkpoint material after each three-unit band. Treat Review, Extra Reading and Let's Talk after Units 1-3, 4-6, and 7-9 as first-class checkpoint lessons, not as part of Unit 3, 6, or 9. Let's Talk exists in Levels 4-6 only — it is the band's functional-dialogue page, so it is practice rather than assessment and takes the activity tone, not the review one. In the Neritan Teacher Menu they render as planned checkpoint rows under the roster whenever the selected unit is the last of its band (3, 6, or 9) and that unit has real content. Until their deck/app files are generated, show them as planned checkpoint cards rather than broken lesson links.
...
Checkpoint learner apps use the same assignment/review loop as unit apps. Use teacher components `review`, `extra-reading` and `lets-talk`, with learner counterparts `review-app`, `extra-reading-app` and `lets-talk-app`. Source tags use band notation such as `OW4-R7-9`, `OW4-ER7-9` and `OW4-LT7-9`.
...
The build id is inlined into both bundles at build time by `next.config.mjs` — the Vercel commit sha in production, `local-<timestamp>` otherwise. `/api/build` is `force-dynamic` and `no-store`, so it answers with the id of whichever deployment the alias points at *now*; a mismatch means the tab is behind and nothing else. It reads `VERCEL_GIT_COMMIT_SHA` at runtime rather than recomputing an id, because a server answering a different string for the same deployment would make the prompt nag forever instead of never.

**That route did not exist for a long time, and nothing noticed.** `NewVersionPrompt` fetched it, got a 404, and returned early on `!response.ok` — so no tab was ever told it was out of date, and several shipped fixes looked exactly like fixes that had not shipped. A check that fails open is a check that is not running: if you add another one, assert the happy path in a real build, not just that nothing threw.

**A lesson frame says which engine it is running.** `ENGINE_REV` in `test-engine.js` prints at the foot of the Answer Section beside the app's build id, because a lesson is a static file inside a frame and "is this the code I just shipped?" otherwise has no answer you can see. Bump it whenever the engine's behaviour changes. Checks run on mount, on focus and on visibility, throttled to one a minute and backed by a 15-minute interval; a dismissal is remembered per build id, so it stays gone until the next deploy. A `local-` id never checks at all, since it changes on every restart and would nag for nothing.
...
Unit 9 ("The Science of Fun") is complete as a unit — opener, vocab-1, song, grammar-1, vocab-2, grammar-2, reading, writing, mission, project, and book-reading all have teacher + learner pairs. The Units 7-9 checkpoint band is complete too — Review 7-9, Extra Reading 7-9 (Leonardo da Vinci), Let's Talk 7-9 (showing interest and asking for help, SB pp.166-167) and the Units 7-9 mastery test all have a teacher deck and a Leo app. The Level 4 final (Units 1-9, `checkpoint-1-9/`) is built as well, and Unit 9 has its own quiz (`unit-9/lessons/quiz.teacher.json`), so Level 4's assessment shelf carries all three kinds: a unit quiz, a band test and the level final. Level 4 Unit 7 ("Good Idea!") is complete. Its vocabulary and grammar scans (`unit-7/vocabulary.json`, `unit-7/grammar.json`) and all ten component pairs are registered in `src/data/lessons.ts`: opener, vocab-1, song, grammar-1, vocab-2, grammar-2, reading, writing, mission, and project. See the Unit 7 build status table in `docs/design-decisions.md` for the file map and each Leo app's module shape.
...
For checkpoint content, the Reference source tree nests it under the level band after the unit entries, for example `Level 4 -> Units 7-9 -> Review 7-9`, `Extra Reading 7-9` and `Let's Talk 7-9`. New checkpoint vocabulary, glossary, academic, reading, and grammar-support items should keep their `OW<level>-R<start>-<end>`, `OW<level>-ER<start>-<end>` or `OW<level>-LT<start>-<end>` source tags.
...
own quiz is `quiz`, a three-unit band test is `test`, a whole-level final is
`final-test` — Level 4 Unit 9 carries all three. Every surface that pairs a
teacher lesson with Leo's app pairs by component, so one shared component would
point every teacher card at the same app. Use `isTestComponent()` from
`src/data/lessons.ts` rather than listing them.

Only two of them are checkpoint material. **A unit quiz belongs to its unit** and
lives in that unit's own `lessons/` folder, which is why `quiz` is absent from
`CHECKPOINT_COMPONENTS`. A band test and a final live in a checkpoint folder
named for what they cover (`checkpoint-7-9/`, `checkpoint-1-9/`), and a new
checkpoint folder must be added to `lessonsDirs` in
`scripts/validate-content.mjs` in the same change.
...
`public/components/test-engine.js`, shared by all of them. Each test is a data
file beside its own pictures (`public/tests/<course>/level-<n>/<slug>/questions.json`)
and a 16-line shell that names it:
...
It was one self-contained HTML file per test until the third one, when ~950 of
each file's ~1,250 lines turned out to be the same engine copied verbatim — so
every fix had to land three times or the copies drifted. **Adding a test is now
writing its questions and nothing else.**

The six kinds between them cover every ExamView section seen so far, across a
unit quiz, a three-unit mastery test and a nine-unit final: `select` (a word
bank), `buttons` (pick one), `multi` (pick two), `text` (typed), `writing`,
`speaking`. Grow the engine only when a paper genuinely needs a shape it has
not got, and add the flag to `docs/tests.md` in the same change. A `text` key
may be a list of accepted wordings, because the publisher prints alternatives
for some transformations; a `writing` part with two prompts is a choice and with
one is just the question.

`scripts/validate-content.mjs` reads each questions file and checks it against
the registry: the storage prefix, homework id and lesson id it claims, its part
count and names against `moduleCount` / `moduleLabels`, its points and question
numbers against the `assessment` block, its minutes against the same, and that
every picture path is absolute and actually in the repo. Those used to be kept
by hand in three places at once.
