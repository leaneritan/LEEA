...
Unit 9 ("The Science of Fun") is complete as a unit — opener, vocab-1, song, grammar-1, vocab-2, grammar-2, reading, writing, mission, project, and book-reading all have teacher + learner pairs. The Units 7-9 checkpoint band is complete too — Review 7-9 and Extra Reading 7-9 (Leonardo da Vinci) both have a teacher deck and a Leo app. Level 4 Unit 7 is the next build target: it has an opener pair only, so it needs the full per-unit pipeline in `docs/build-order.md` starting with the vocabulary and grammar scans.
