...
Unit 9 ("The Science of Fun") is complete as a unit — opener, vocab-1, song, grammar-1, vocab-2, grammar-2, reading, writing, mission, project, and book-reading all have teacher + learner pairs. The Units 7-9 checkpoint band is complete too — Review 7-9 and Extra Reading 7-9 (Leonardo da Vinci) both have a teacher deck and a Leo app. Level 4 Unit 7 ("Good Idea!") is the current build target. Its vocabulary and grammar scans are done (`unit-7/vocabulary.json`, `unit-7/grammar.json`), and it now has two registered pairs: **opener** and **vocab-1** (teacher deck `public/lessons/ow-l4-u7-vocab-1.html`, 39 slides with a bespoke mini-game per target word; Leo app `public/learn/ow-l4-u7-vocab-1.html`, the locked 13-tab vocab shell). Remaining per `docs/build-order.md`: song, grammar-1, vocab-2, grammar-2, reading, writing, mission, project.
