...
**`examples[].highlight` coverage is required for every sample sentence, not just the ones shown above.** `examples` is not scoped to `tab1_samples` alone — it must also cover every sentence in `tab2_levelup.mixed_samples` (and any other sample-sentence list the grammar point renders). `GrammarCard.tsx`'s `highlightGrammarPhrase()` looks up the highlight by exact sentence-text match via `GrammarEntry.highlightsBySentence`; any sample sentence missing a matching `examples` entry renders with no color-coded phrase. When authoring or scanning a grammar point, build the full sentence list first (both tabs), then add one `examples` entry per sentence with the substring that demonstrates the grammar point.

**`scripts/validate-content.mjs` now enforces this coverage** — it fails the build if any `tab1_samples` or `tab2_levelup.mixed_samples` sentence has no matching `examples[].sentence` entry. This wasn't always checked: Unit 6 shipped with 4 such gaps (2 per grammar point) that went unnoticed until the check was added, because a missing highlight is a silent visual gap, not a build error. If the validator flags this, add the missing `examples` entry rather than treating it as a false positive.
...
**Every grammar point should get role-based color coding** (subject/verb/direct-object/etc, matching the colors already used for pattern-chart chips). **Default: keep `chart.table` as the primary view and color its cells** — tag `chart.table.rows[].roles` (one entry per cell, matching `cells` order) and optionally `rows[].labelRole`, using one of the `GrammarRoleKey` values in `src/data/types.ts`: `subject` / `verb` / `directObject` / `indirectObject` / `prep` / `clause` / `cause` / `effect` / `obligation` / `prohibition` / `futurePositive` / `futureNegative`. `clause` is the neutral/general-content color — use it for cells that carry grammar content but don't map to a specific role, or `null` for cells that shouldn't be tinted at all. `GrammarTableChart` shows a legend (only for roles actually used) and tints each cell accordingly, replacing the old single-color `highlight` (which still works as a fallback when `roles` isn't set).

**A role only gets color if it has a matching CSS class and `CHART_LEGEND` entry — nothing enforces this, so it fails silently.** `GrammarTableChart` applies `gcardv2-table-cell--role-${role}` directly from whatever string is in the JSON; if that class isn't defined in `globals.css`, the cell renders with no color at all and no error. This exact gap shipped in Units 6 and 9: `obligation`/`prohibition`, `future-positive`/`future-negative`, and `cause`/`effect` were all used as table roles with no matching CSS, so those tables rendered as plain gray while Unit 8's canonical-role table (subject/verb/directObject/indirectObject) looked properly color-coded. **If the pattern you're scanning doesn't fit the six original roles, check `GrammarRoleKey` first — if it's not listed, add the new key to the type in `src/data/types.ts`, a CSS rule `.gcardv2-table-cell--role-<key>` in `globals.css`, and a `CHART_LEGEND` entry in `reference-shapes.ts`, all in the same change.** Don't invent a role string without doing all three.

A separate chip/legend view exists (`chart.table.preferChips: true`, rendered by `PatternChart()`'s chip branch) for pure S-V-O sentence patterns, but it's a secondary option — don't switch a point away from the table view to chips unless asked. Prefer tinting the existing table.
