...
成績の使い方のルール：
- 会話の自然な流れの中で（毎回ではなく、話が一区切りしたときや、成長に触れるのがふさわしいタイミングで）、この成績を具体的な数字つきで励ましに使ってよい。例：「前回のクイズは75%だったから、今回85%を目指してみようか」のように、実際の数字にもとづいて、次に届きそうな目標を示す。
...
/** Renders Leo's recent quiz history into plain text so the tutor can give concrete, numeric encouragement. */
function summarizeAttempts(attempts: QuizAttempt[] | undefined): string {
  if (!Array.isArray(attempts) || attempts.length === 0) return "";

  const lines = attempts.slice(0, 5).map((a) => {
