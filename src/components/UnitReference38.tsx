...
import {
  level3Unit8AcademicItems,
  level3Unit8GlossaryItems,
  level3Unit8Vocab1Items,
  level3Unit8Vocab2Items
} from "@/data/reference";
...
const unitSections: Section[] = [
  {
    id: "vocab1",
    title: "Vocabulary 1",
    sub: "Celebrating, costumes, parading & festivity",
    icon: "①",
    accent: "var(--good)",
    tint: "var(--good-tint)",
    words: level3Unit8Vocab1Items.map(toWordEntry)
  },
  {
    id: "vocab2",
    title: "Vocabulary 2",
    sub: "Balloons, birthday cakes & party details",
    icon: "②",
    accent: "#2f9c8e",
    tint: "#e6f4f1",
    words: level3Unit8Vocab2Items.map(toWordEntry)
  },
  {
    id: "academic",
    title: "Academic",
    ...
    words: level3Unit8AcademicItems.map(toWordEntry)
  },
  {
    id: "glossary",
    title: "Glossary",
    sub: "Reading, values & reader terms",
    icon: "📖",
    accent: "var(--muted-2)",
    tint: "#f0f1ec",
    words: level3Unit8GlossaryItems.map(toWordEntry)
  }
].filter(s => s.words.length > 0);

const unitGrammarEntries = allGrammar
  .filter((g) => g.course === "our-world" && g.level === 3 && g.unit === 8)
...
const jumps = [
  ...
  ...(unitGrammar.length > 0 ? [{ label: "Grammar", count: unitGrammar.length, dot: "var(--accent)", href: "#grammar" }] : [])
];
...
export default function UnitReference38() {
  return (
          ...
          <div className="unit-hero-left">
            ...
            <div>
              <div className="unit-hero-eyebrow-row">
                <span className="unit-hero-eyebrow">Our World · Level 3 · Unit 8</span>
              </div>
              <h1 className="unit-hero-title">Let&apos;s Celebrate</h1>
            </div>
          </div>
